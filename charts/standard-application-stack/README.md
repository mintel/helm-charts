...
| apiGateway.appName | string | `""` | The app name portion of the API URL as decided between you and the Architecture team. For instance, for https://api.mintel.com/content/generative-answer/docs this would be "generative-answer". |
| apiGateway.enabled | bool | `false` | Whether to configure the app to receive traffic from an API gateway. Note that this only exposes your endpoint; further configuration of the API gateway itself is required (speak to Infra). |
...
| ingress.alb.conditions | list | `[{"condition":"","targetService":""}]` | For specifying routing conditions in addition to the standard host/path condition (see https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.7/guide/ingress/annotations/#conditions) |
