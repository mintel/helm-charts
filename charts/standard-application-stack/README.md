...
| autoscaling | object | `{"advanced":{},"cooldownPeriod":300,"enableZeroReplicas":false,"enabled":false,"fallback":{"failureThreshold":3},"maxReplicaCount":5,"mimir":{"customHeaders":"","enabled":true,"serverAddress":""},"minReplicaCount":2,"pollingInterval":30,"scaleTargetRef":{"apiVersion":"apps/v1","envSourceContainerName":"","kind":"Deployment"},"triggers":{}}` | Handle autoscaling via https://keda.sh Creates a ScaledObject for the workload ref: https://keda.sh/docs/2.8/concepts/scaling-deployments/ |
| autoscaling.mimir.customHeaders | string | `""` | Optional custom headers to send to Mimir |
| autoscaling.mimir.enabled | bool | `true` | Enable Mimir integration for KEDA prometheus triggers |
| autoscaling.mimir.serverAddress | string | `""` | Mimir server address (e.g. https://mimir.monitoring.svc:8080) |
...
| cronjobs | object | `{"defaults":{"concurrencyPolicy":"Forbid","enableDoNotDisrupt":true,"restartPolicy":"Never","suspend":false,"timezone":null,"ttlSecondsAfterFinished":600},"jobs":[]}` | Define and Configure CronJob's Defaults to same image as main deployment but with defined arguments |
| cronjobs.defaults | object | `{"concurrencyPolicy":"Forbid","enableDoNotDisrupt":true,"restartPolicy":"Never","suspend":false,"timezone":null,"ttlSecondsAfterFinished":600}` | Defaults for all CronJob's |
